...
namespace OpenTKLib
{
    public class PointCloudVertices : List<Vertex>// PointCloudGL //List<Vertex>
    {
        ...
        public List<Line> LinesGeometricalModels;
        public List<Line> LinesNormals;
        ...
        private List<Line> linesPCAAxes;
        ...
        public static PointCloudVertices SubtractVector(PointCloudVertices pointCloud, Vector3d centroid)
        {
            ...
            for (int i = 0; i < pointCloud.Count; i++)
            {
                ...
                Vertex v = pointCloud[i];
                Vector3d translatedV = Vector3d.Subtract(v.Vector, centroid);
                resultCloud.Add(new Vertex(v.IndexInModel, translatedV));
        ...
        public PointCloudVertices SubtractClouds(PointCloudVertices pointCloud)
        {
            ...
            for (int i = 0; i < this.Count; i++)
            {
                ...
                Vertex v = pointCloud[i];
                resultCloud.Add(new Vertex(v.IndexInModel, this[i].Vector - pointCloud[i].Vector));
        ...
        public static PointCloudVertices CopyVertices(PointCloudVertices pointsTarget)
        {
            ...
            for (int i = 0; i < pointsTarget.Count; i++)
            {
                ...
                Vertex v = new Vertex(point1.IndexInModel, point1.Vector, point1.Color);
        ...
        {
            PointCloudVertices points = new PointCloudVertices();
            for (int i = 0; i < pcl.Vectors.GetLength(0); i++)
            {
            ...
            points.FilaName = pcl.FileNameLong;
        ...
        {
            PointCloudVertices pcNew = new PointCloudVertices();

            for (int i = 0; i < this.Count; i++)
            {
                Vector3d v = this[i].Vector;
                Vector3d vNew = new Vector3d(v);
                pcNew.Add(new Vertex(i, vNew));
            }
            return pcNew;
        }
        ...
        public static PointCloudVertices CloneVertices(PointCloudVertices myOldList)
        {
            ...
            for (int i = 0; i < myOldList.Count; i++)
            {
                ...
                Vertex vNew = new Vertex(v.IndexInModel, v.Vector);
        ...
        public static float MeanDistance(PointCloudVertices a, PointCloudVertices b)
        {
            ...
            float meanDistance = totaldist / Convert.ToSingle(a.Count);
            return meanDistance;
        ...
        public List<Line> LinesPCAAxes
        {
            get
            {
                if(this.linesPCAAxes == null)
                    CalculateLinesPCA();
                return linesPCAAxes;
        ...
        public void CalculateLinesPCA()
        {
            //if (PCAAxes == null)
            //    return;

            PCAAxes = new PointCloudVertices();
            PCA pca = new PCA();
            pca.PCA_OfPointCloud(this);
            linesPCAAxes = new List<Line>();
            System.Drawing.Color color = new System.Drawing.Color();
            

            for (int i = 0; i < 3; i++)
            {
                Vertex v = this.PCAAxes[i];

                if (i == 0)
                    color = System.Drawing.Color.FromArgb(1, 1, 1);
                if (i == 1)
                    color = System.Drawing.Color.FromArgb(255, 255, 0);
                    
                if (i == 2)
                    color = System.Drawing.Color.FromArgb(255, 0, 255);

               
                Line line = new Line(new Vertex(this.CentroidVector - v.Vector).Vector, new Vertex(v.Vector + CentroidVector).Vector);
                line.Color = color;
                linesPCAAxes.Add(line);
        ...
        public void LinesGeometricalModels_Reset()
        {
            ...
            this.LinesGeometricalModels = new List<Line>();
            ...
            for (int i = 0; i < this.Count; i++)
            {
                ...
                for (int j = 0; j < this.Count; j++)
                {
                    if (i != j)
                    {
                        ...
                        //if (dist == cubeSize)
                        {
                            Line myLine = new Line(this[i].Vector, this[j].Vector, Color);
        ...
        //{
        //    List<Vector3d> myVectors = PointCloud.Vector3dDList_FromArray(depthInfo, width, height);
        //    PointCloud pc = PointCloud.FromVectors(myVectors);
        ...
        public PointCloud ToPointCloud()
        {
            ...
            pcgl.FileNameLong = this.FilaName;
            pcgl.Path = this.Path;
